export function matchesDomain(currentDomain: string, pattern: string): boolean {
  ...
  if (pattern.startsWith('*.')) {
    ...
    return (
      currentDomain.endsWith('.' + baseDomain) || currentDomain === baseDomain
    );
  }
...
export async function findMatchingFormat(
  currentDomain: string
): Promise<string> {
  try {
    const allSettings = (await chrome.storage.sync.get(null)) as Record<
      string,
      string
    >;
